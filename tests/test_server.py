...
from ifwebuildit.server import create_app


@pytest.fixture
def client():
    """Create a test client with mocked storage."""
    with patch("ifwebuildit.server.StorageService") as mock_storage_class:
...
def test_chat_baseline_with_service():
    ...
    with (
        patch("ifwebuildit.server.StorageService"),
        patch("ifwebuildit.server.SearchService"),
        patch("ifwebuildit.server.EmbeddingService"),
        patch("ifwebuildit.server.ChatService") as mock_chat_cls,
        patch("ifwebuildit.server.get_settings") as mock_settings,
    ):
        settings = MagicMock()
