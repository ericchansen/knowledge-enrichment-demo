# ifwebuildit — Knowledge Base Overnight Enrichment
...
# Start the server
uv run uvicorn ifwebuildit.server:app --reload
```

Open http://localhost:8000 for the side-by-side comparison UI.
...
```bash
docker build -t ifwebuildit .
docker run -p 8000:8000 --env-file .env ifwebuildit
```
